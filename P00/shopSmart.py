...
def shopSmart(orderList, fruitShops):
    ...
    """
    "*** YOUR CODE HERE ***"
    currentShop = fruitShops[0]  # initialize

    # getPriceOfOrder returns sum of orders from function in shop.py
    currentCheapestOne = currentShop.getPriceOfOrder(orderList)

    for shop in fruitShops[1:]:
        cost = shop.getPriceOfOrder(orderList)
        if cost < currentCheapestOne:
            currentShop = shop
            currentCheapestOne = cost
